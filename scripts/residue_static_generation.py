...
def process_residue(hdffile, index, weight_type, settings):
    with feater.io.hdffile(hdffile, "r") as hdf:
        ...
        weights = hdf["elements"][
            crd_start:crd_end
        ]  # Get atom mass for determination of atom type
    ...
    ret_data = feater.voxelize.interpolate(
        coord, weights, dims, spacing=spacing, cutoff=cutoff, sigma=sigma
    )
...
def split_dataset(datafile, trainingset_nrs, output_folder):
        ...
        for tagi in range(class_nr):
            ...
            if len(locations) > needed_nr:
                ...
                for bin_idx, target_nr in enumerate(trainingset_nrs):
                    ...
                    selected_restypes[bin_idx] += [
                        feater.LAB2RES_DUAL[tagi]
                    ] * target_nr
            else:
                ...
                for bin_idx, _target_nr in enumerate(trainingset_nrs):
                    ...
                    selected_restypes[bin_idx] += [feater.LAB2RES_DUAL[tagi]] * len(
                        bini
                    )
    ...
    if len(output_folder) > 0:
        ...
        for i in range(len(trainingset_nrs)):
            ...
            print(
                f"Rank {i} has {len(selected_final[i])} entries; Writing to {csv_out} ..."
            )
            df.to_csv(csv_out, index=False)
    else:
        print(
            f"Rank {i} has {len(selected_final[i])} entries; Skipping the writing ..."
        )
...
def parse_args():
    ...
    parser.add_argument(
        "-f", "--datasetfile", type=str, default="", help="Dataset to use"
    )
    parser.add_argument(
        "-c", "--cpu_nr", type=int, default=16, help="Number of CPUs to use"
    )
    parser.add_argument(
        "-b", "--batch_nr", type=int, default=1024, help="Number of batches to use"
    )
    parser.add_argument("--start_batch", type=int, default=0, help="Start batch")
    parser.add_argument(
        "-o", "--output_dir", type=str, default="/tmp/", help="Output directory"
    )
    parser.add_argument("--weight_type", type=str, default="mass", help="Weight type")
    parser.add_argument(
        "--target_nr", type=int, default=1000, help="Target number of residues"
    )
...
def process_dataset(
    csvfile, source_data, output_hdf, weight_type, start_batch, batch_nr, cpu_nr
):
    ...
    if start_batch == 0:
        with feater.io.hdffile(output_hdf, "w") as f:
            ...
            feater.utils.add_data_to_hdf(
                f,
                "boxsize",
                np.array([VOX_SETTINGS["boxsize"]], dtype=np.float32),
                maxshape=[1],
            )
    else:
    ...
    for b in range(start_batch, len(batches)):
        ...
        print(
            f"Finished batch {b + 1}/{batch_nr} with {len(batchi)} residues; Used {time.perf_counter() - st:.2f} seconds"
        )
...
if __name__ == "__main__":
    ...
    START_BATCH = (
        0  # Index of the start batch (incase of a restart); 0 for new generation
    )
    WEIGHT_TYPE = (
        args.weight_type
    )  # Types of weight to pass to cuda code: either "mass" or "radius"
    ...
    VOX_SETTINGS = {"dims": [32, 32, 32], "boxsize": 16.0, "cutoff": 4.0, "sigma": 1.5}
