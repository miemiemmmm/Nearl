...
if __name__ == "__main__":
    ...
    def load_data(dirname, residue_order):
        ...
        for res in residue_order:
            ...
            if np.isnan(datai).any() or np.isinf(datai).any():
                ...
                print(
                    "Nan detected in the data file for residue: ",
                    res,
                    " ; Number of nan: ",
                    correction,
                )
        ...
        for res in residue_order:
            ...
            ret_data[start_row : start_row + entry_number, :] = np.asarray(
                datai, dtype=np.float64
            )
    ...
    if not os.path.isfile(os.path.join(outfolder, "ref_arr.npy")):
        ...
        result_label = np.array([thedict[res] for res in ref_result])[
            :, np.newaxis
        ].astype(np.float32)
        ...
        print(
            f"The number of outliers: {np.count_nonzero(~mask1)}; Final result shape: {ref_arr.shape}, {result_label.shape}"
        )

    ref_arr = np.load(os.path.join(outfolder, "ref_arr.npy"), allow_pickle=True)
    result_label = np.load(
        os.path.join(outfolder, "result_label.npy"), allow_pickle=True
    )

    testset_arr = np.load(os.path.join(reffolder, "ref_arr.npy"), allow_pickle=True)
    testset_label = np.load(
        os.path.join(reffolder, "result_label.npy"), allow_pickle=True
    )
    ...
    dataset = TensorDataset(
        torch.from_numpy(training_data).to(DEVICE, dtype=torch.float32),
    ...
    for epoch in range(EPOCHS):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            if ((i + 1) % 100) == 0:
                print(
                    f"Epoch{epoch + 1}, batch{i + 1:5d}: running loss: {running_loss:.3f}"
                )
                running_loss = 0.0
    print(
        "Finished Training, Total time elapsed: ", time.perf_counter() - st, " seconds"
    )
    ...
    with torch.no_grad():
        ...
        print(
            f"Accuracy of the network on dataset ({correct}/{total}); Percentage rate: {100 * (correct / total)}%"
        )
    ...
    with torch.no_grad():
        ...
        for data in _dataloader:
            ...
            correct += (outputs == labels).sum().item()
            coords = (
                torch.stack((labels, outputs), dim=1)
                .cpu()
                .numpy()
                .reshape((-1, 2))
                .astype(np.int32)
        ...
        print(
            f"Accuracy of the network on the ({correct}/{total}); Percentage rate: {100 * (correct / total)}%"
        )
