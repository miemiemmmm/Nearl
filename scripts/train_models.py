...
def draw_scatter(
    ...
    if fig is None:
        f = sns.JointGrid(
            data=df, x="groundtruth", y="predicted", xlim=(1, 13), ylim=(1, 13)
        )
    else:
        f = fig
    if figtype == "scatter":
        ...
        f.ax_joint.kdeplot(
            df["groundtruth"], df["predicted"], levels=11, cmap="inferno", thresh=0
        )
    else:
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The file writes all of the absolute path of h5 files of training data set",
    )
    parser.add_argument(
        "-test",
        "--test_data",
        type=str,
        required=True,
        help="The file writes all of the absolute path of h5 files of test data set",
    )
    parser.add_argument(
        "-o", "--output_folder", type=str, required=True, help="Output folder"
    )
    parser.add_argument(
        "-od",
        "--output_dimension",
        type=int,
        required=True,
        help="Output dimension, 1 for regression, >1 for classification",
    )
    parser.add_argument(
        "-w",
        "--data_workers",
    ...
    parser.add_argument(
        "--test_number", type=int, default=4000, help="Number of test samples to use"
    )
    parser.add_argument(
        "-t", "--tags", type=str, default="", help="Feature named delimited by '%' "
    )
    parser.add_argument(
        "-l", "--labeltag", type=str, default="label", help="Label name"
    )
    ...
    parser.add_argument(
        "-lr", "--lr-init", type=float, default=0.001, help="Learning rate"
    )
    parser.add_argument(
        "--lr-decay-steps",
        type=int,
        default=30,
        help="Decay the learning rate every n steps",
    )
    parser.add_argument(
        "--lr-decay-rate",
        type=float,
        default=0.5,
        help="Decay the learning rate by this rate",
    )

    # Other parameters
    parser.add_argument(
        "-v", "--verbose", default=0, type=int, help="Verbose printing while training"
    )
    parser.add_argument("--manualSeed", type=int, help="Manually set seed")
    parser.add_argument(
        "--cuda", type=int, default=int(torch.cuda.is_available()), help="Use CUDA"
    )
    ...
    if not os.path.exists(args.training_data):
        raise FileNotFoundError(
            f"The training data file {args.training_data} does not exist"
        )
...
def perform_training(training_settings: dict):
    ...
    dimensions = training_settings[
        "dimensions"
    ]  # TODO: this has to match the voxel or image size
    ...
    training_data = Dataset(
        trainingfiles, feature_keys=datatags, label_key=labeltag, normalize=False
    )  # TODO: Check the normalization
    test_data = Dataset(
        testfiles, feature_keys=datatags, label_key=labeltag, normalize=False
    )  # TODO: Check the normalization
    ...
    optimizer = OPTIMIZERS[training_settings["optimizer"]](
        model.parameters(), lr=training_settings["lr_init"], **optim_kwargs
    )

    if training_settings["loss_function"] not in LOSS_FUNCTIONS:
        raise ValueError(
            f"Loss function {training_settings['loss_function']} is not supported"
        )
    ...
    scheduler = optim.lr_scheduler.StepLR(
        optimizer,
        step_size=training_settings["lr_decay_steps"],
        gamma=training_settings["lr_decay_rate"],
    )
    ...
    for epoch in range(EPOCH_NR):
        ...
        if epoch < START_EPOCH:
            ...
            continue
        message = (
            f" Running the epoch {epoch + 1: >4d}/{EPOCH_NR: <4d} at {time.ctime()} "
        )
        print(f"{message:#^80}")
        batch_nr = (len(training_data) + BATCH_SIZE - 1) // BATCH_SIZE
        for batch_idx, batch in enumerate(
            training_data.mini_batches(
                batch_size=BATCH_SIZE, process_nr=WORKER_NR, augment=AUGMENT
            )
        ):
            ...
            if (batch_idx + 1) % (batch_nr // 3) == 0 and (
                not training_settings["production"]
            ):
                preds_tr, targets_tr, losses_tr = nearl.utils.test_model(
                    model,
                    training_data,
                    criterion,
                    training_settings["test_number"],
                    BATCH_SIZE,
                    USECUDA,
                    WORKER_NR,
                )
                preds_te, targets_te, losses_te = nearl.utils.test_model(
                    model,
                    test_data,
                    criterion,
                    training_settings["test_number"],
                ...
                if True in np.isnan(preds_te):
                    print("Warning: Found NaN in the test prediction")
                    continue
                elif True in np.isnan(preds_tr):
                    print("Warning: Found NaN in the training prediction")
                    continue

                if output_dims == 1:
                    # Metrics on the training set
                    preds_tr, targets_tr, losses_tr = (
                        preds_tr.flatten(),
                        targets_tr.flatten(),
                        losses_tr.flatten(),
                    )
                    ...
                    # Accuracy
                    accuracy_tr = np.count_nonzero(preds_tr == targets_tr) / len(
                        preds_tr
                    )
                    accuracy_te = np.count_nonzero(preds_te == targets_te) / len(
                        preds_te
                    )
                ...
                if (training_settings["verbose"] > 0) or (
                    not training_settings["production"]
                ):
                    # Add the performance to the tensorboard
                    tensorboard_writer.add_scalar(
                        "Loss/Train", np.mean(losses_tr), epoch * batch_nr + batch_idx
                    )
                    tensorboard_writer.add_scalar(
                        "Loss/Test", np.mean(losses_te), epoch * batch_nr + batch_idx
                    )
                    if output_dims == 1:
                        # RMSE
                        tensorboard_writer.add_scalar(
                            "Perf/RMSE", rmse_te, epoch * batch_nr + batch_idx
                        )
                        tensorboard_writer.add_scalar(
                            "Perf/RMSE*", rmse_tr, epoch * batch_nr + batch_idx
                        )
                        tensorboard_writer.add_scalar(
                            "Perf/R", pearson_te, epoch * batch_nr + batch_idx
                        )
                        tensorboard_writer.add_scalar(
                            "Perf/Rho", spearman_te, epoch * batch_nr + batch_idx
                        )
                    elif output_dims > 1:
                        # Accuracy
                        tensorboard_writer.add_scalar(
                            "Perf/Train", accuracy_tr, epoch * batch_nr + batch_idx
                        )
                        tensorboard_writer.add_scalar(
                            "Perf/Test", accuracy_te, epoch * batch_nr + batch_idx
                        )

                    fig_tr = draw_scatter(
                        preds_tr, targets_tr, figtype="scatter", color=COLORS[1]
                    )
                    fig_te = draw_scatter(
                        preds_te,
                        targets_te,
                    ...
                    tensorboard_writer.add_figure(
                        "Dist/Results", fig_te.fig, epoch * batch_nr + batch_idx
                    )
                    ...
                    for idx, p in enumerate(parmset):
                        ...
                        if p.grad is not None:
                            ...
                            tensorboard_writer.add_scalar(
                                f"GradNorm/{pname}_{idx}",
                                grad_norm,
                                epoch * batch_nr + batch_idx,
                            )
                            tensorboard_writer.add_histogram(
                                f"GradHist/{pname}_{idx}",
                                p.grad,
                                epoch * batch_nr + batch_idx,
                            )
        ...
        if output_dims == 1:
            ...
            pearson_on_train, spearman_on_train = compute_correlations(
                targets_tr, preds_tr
            )
            pearson_on_test, spearman_on_test = compute_correlations(
                targets_te, preds_te
            )
        else:
        ...
        print(
            f"Checking the Performance on Loss: {loss_on_test}/{loss_on_train}; Accuracy: {accuracy_on_test}/{accuracy_on_train} at {time.ctime()}"
        )

        # Save the model
        modelfile_output = os.path.join(
            os.path.abspath(training_settings["output_folder"]),
            f"{MODEL_TYPE}_{epoch}.pth",
        )
        ...
        fig_te = draw_scatter(
            preds_te, targets_te, figtype="scatter", fig=fig_tr, color=COLORS[0]
        )
        fig_te.savefig(
            os.path.join(training_settings["output_folder"], f"Results_{epoch}.png")
        )
        ...
        # Save the metrics
        with h5.File(
            os.path.join(training_settings["output_folder"], "performance.h5"), "a"
        ) as hdffile:
            nearl.utils.update_hdf_data(
                hdffile,
                "loss_train",
                np.array([loss_on_train], dtype=np.float64),
                np.s_[epoch : epoch + 1],
                dtype=np.float64,
                maxshape=(None,),
            )
            nearl.utils.update_hdf_data(
                hdffile,
                "loss_test",
                np.array([loss_on_test], dtype=np.float64),
                np.s_[epoch : epoch + 1],
                dtype=np.float64,
                maxshape=(None,),
            )
            nearl.utils.update_hdf_data(
                hdffile,
                "accuracy_train",
                np.array([accuracy_on_train], dtype=np.float64),
                np.s_[epoch : epoch + 1],
                dtype=np.float64,
                maxshape=(None,),
            )
            nearl.utils.update_hdf_data(
                hdffile,
                "accuracy_test",
                np.array([accuracy_on_test], dtype=np.float64),
                np.s_[epoch : epoch + 1],
                dtype=np.float64,
                maxshape=(None,),
            )
            if output_dims == 1:
                nearl.utils.update_hdf_data(
                    hdffile,
                    "pearsonr_train",
                    np.array([pearson_on_train], dtype=np.float64),
                    np.s_[epoch : epoch + 1],
                    dtype=np.float64,
                    maxshape=(None,),
                )
                nearl.utils.update_hdf_data(
                    hdffile,
                    "pearsonr_test",
                    np.array([pearson_on_test], dtype=np.float64),
                    np.s_[epoch : epoch + 1],
                    dtype=np.float64,
                    maxshape=(None,),
                )
                nearl.utils.update_hdf_data(
                    hdffile,
                    "spearmanr_train",
                    np.array([spearman_on_train], dtype=np.float64),
                    np.s_[epoch : epoch + 1],
                    dtype=np.float64,
                    maxshape=(None,),
                )
                nearl.utils.update_hdf_data(
                    hdffile,
                    "spearmanr_test",
                    np.array([spearman_on_test], dtype=np.float64),
                    np.s_[epoch : epoch + 1],
                    dtype=np.float64,
                    maxshape=(None,),
                )
...
if __name__ == "__main__":
    ...
    SETTINGS["datatags"] = [
        i.strip() for i in SETTINGS["tags"].split("%") if len(i.strip()) > 0
    ]
    ...
    if (args.verbose > 0) or (not args.production):
        ...
        tensorboard_writer = SummaryWriter(
            os.path.join(SETTINGS["output_folder"], "tensorboard")
        )
