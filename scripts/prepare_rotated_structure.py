...
if __name__ == "__main__":
  ...
  """
    ...
    rotation_steps = (
        20  # For each degree of freedom, how many steps to take for one full rotation
    )
    translate = [
        0,
        0,
    ...
    if to_origin:
        print(
            f"Move the system to the origin point, offset is {np.mean(coord, axis=0)}"
        )
        coord -= np.mean(coord, axis=0)

    results = parallelize_transform(euler_angles)
    print(
        f"Finished computing new frames, start writing to trajectory file, time elapsed: {time.perf_counter() - st}"
    )
    # Put the new frames to a pytraj trajectory
    st_write = time.perf_counter()
    new_traj = pt.Trajectory(
        xyz=np.array([frame.xyz for frame in results]), top=traj.top
    )
    print(
        f"Finished putting frames to pytraj trajectory, time elapsed: {time.perf_counter() - st_write}"
    )
