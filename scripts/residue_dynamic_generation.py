...
def parse_args():
    ...
    parser.add_argument(
        "-f", "--pdbcodes", type=str, help="The file containing the list of pdb codes"
    )
    parser.add_argument(
        "-d",
        "--misato_dir",
...
def generate_features(settings):
    ...
    outputfile = os.path.join(
        os.path.abspath(output_dir), f"MisatoOutput{task_index}.h5"
    )
    print(
        f"Input file: {pdbcode_file}, Output file: {outputfile}; Task {task_index} of {task_nr}"
    )
    ...
    features["feat_dyna"] = nearl.features.MarchingObservers(
