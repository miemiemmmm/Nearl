...
import nearl
from nearl import _tempfolder, features, log, utils
...
def parallelize_traj(traj_list):
    ...
    for trajidx, trajectory in enumerate(traj_loader):
        print(
            f"Processing trajectory {trajectory.top_filename}, Last structure took {time.perf_counter() - st} seconds"
        )
        st = time.perf_counter()
        ...
        if (trajidx + 1) % 25 == 0:
            log(
                f"Processed {len(ret_list)}/({len(traj_list)}) trajectories; Time elapsed: {time.perf_counter() - st} seconds"
            )
...
if __name__ == "__main__":
    ...
    if not SKIP_COMBINE:
        ...
        with Client(processes=True, n_workers=24, threads_per_worker=1) as client:
            tasks = [
                dask.delayed(combine_complex)(idx, row, refdir)
                for (idx, row), refdir in zip(table.iterrows(), refdirs)
                if not os.path.exists(
                    os.path.join(out_filedir, f"{row[0]}_complex.pdb")
                )
            ]
        ...
        log(
            f"Complex combination finished. Used {time.perf_counter() - st:.2f} seconds."
        )
    ...
    complex_files = [
        os.path.join(complex_dir, f"{pdbcode.lower()}_complex.pdb")
        for pdbcode in pdb_to_featurize
    ]
    found_state = [os.path.exists(pdbfile) for pdbfile in complex_files]
    if np.count_nonzero(found_state) == len(found_state):
        print(
            f"Congretulations: All complexes found ({np.count_nonzero(found_state)}/{len(found_state)})"
        )
    else:
        print(
    ...
    FEATURIZER_PARMS = {
        ...
        "CUBOID_DIMENSION": [
            ...
            45,
            45,
        ],  # Unit: 1 (Number of lattice in one dimension)
    ...
    if do_test:
        ...
        with Client(cluster) as client, performance_report(filename="dask-report.html"):
            tasks = [
                dask.delayed(parallelize_traj)(traj_list) for traj_list in split_groups
            ]
    ...
    log(
        f"Tasks are finished, Collecting data... Pure computation time: {time.perf_counter() - st_compute:.2f} seconds."
    )
    ...
    for idx, key in keydict.items():
        for tmpfile in results:
            ...
            if key in ["pdbcode"]:
                datai = np.array([s.encode("utf8") for s in datai.ravel()]).reshape(
                    (-1, 1)
                )
    ...
    log(
        f"##################Data are collected {time.perf_counter() - st:.3f} ################"
    )
