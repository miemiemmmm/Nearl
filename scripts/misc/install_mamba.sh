...
micromamba create --name TEST_ENV python=3.9.17 -c conda-forge -q -y
micromamba activate TEST_ENV
if micromamba env list | grep TEST_ENV | grep "*" &> /dev/null; then
  echo "Great! Micromamba installation successful and test environment activated";
  echo -e "Y\n" | micromamba env remove --name TEST_ENV -q -y;
else
  echo "No!!!! Micromamba cannot activate the TEST_ENV environment";
  exit 1
fi

# Create a script to load micromamba
