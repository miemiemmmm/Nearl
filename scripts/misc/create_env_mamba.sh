...
if [ ${#MAMBA_EXE} -gt 10 ]; then
  ...
  logit "MAMBA_EXE is not defined, please check if mamba/micromamba is installed or not";
  exit -1;
fi

${MAMBA_EXE} env list
foundenv=$(${MAMBA_EXE} env list | grep envs | awk '{print $1,$2}' | egrep "/${env_name}$" | awk '{print $NF}')
if [ ${#foundenv} -gt 0 ]; then
  logit "Warning: Found a pre-existing environment with the same name ${foundenv}; Removing it; ";
  echo -e "Y\n" | ${MAMBA_EXE} env remove --name ${env_name};
fi

logit "Creating the environment, installation of AmberTools might take a while ...";
${MAMBA_EXE} create --name ${env_name} -c conda-forge python=3.9.17 ambertools=22.0 -y
eval "$(micromamba shell hook --shell bash)"
logit "Activating the environment"
...
if [[ ${CONDA_DEFAULT_ENV} != ${env_name} ]]; then
  ...
  ${MAMBA_EXE} install -c conda-forge scipy pandas scikit-learn h5py seaborn matplotlib -y
...
${MAMBA_EXE} install -c conda-forge -c anaconda notebook=6.4 ipywidgets=7.6 nglview=3.0.3 -y  # traj visualizer
${MAMBA_EXE} install -c conda-forge requests biopython trimesh -y
