...
table = pd.read_csv("/media/yzhang/MieT5/BetaPose/data/PDBBind_general_v2020.csv", delimiter=",")
pdblist = table.pdbcode.tolist(); 

def misato_ligand_atomnr(pdbcode, misato_qmfile): 
  with data_io.hdf_operator(misato_qmfile) as f1: 
...
with data_io.hdf_operator(qmfile) as f1:
  count = 0
...
# 173 entries has multimer peptide as ligand and inconsistant ligand atom number with QM.
# 70  entries has amino acid as ligand and inconsistant ligand atom number with QM.
# Finally 109 entries cannot match
...
with data_io.hdf_operator(mdfile) as f1:
  with open("/home/yzhang/Downloads/misato_database/train_MD.txt","r") as file_train:
  ...
  for pdbcode in thelist:
    ...
    if pdbcode.lower() in SUPERSEDES: 
...
with open("/tmp/test.json", "w") as f1:
  json.dump(final_lig_dict, f1, indent=2)

with open("/tmp/test.txt", "w") as f1:
...
#     traj = utils.misato_traj(pdbcode, mdfile, parmdir)
#     atomnr_traj = traj.n_atoms
#     if (atomnr!=atomnr_traj):
#       print("Warning: not identical atom number!!!")
    
#     if "mer" not in comment and lig_atomnr not in segment_nrs:
#       c += 1
#       print(f"{c}th inconsistance Ligand has atoms: ", lig_atomnr)
#       print(f"{pdbcode}::{len(segment_nrs)} segments :::: -> ",segment_nrs)
...
# $ /10GS                /
# $ /10GS/atoms_element  : Shape-(6593,)
# $ /10GS/atoms_number   : Shape-(6593,)
# $ /10GS/atoms_residue  : Shape-(6593,)
# $ /10GS/atoms_type     : Shape-(6593,)
# $ /10GS/frames_bSASA   : Shape-(100,)
# $ /10GS/frames_distance: Shape-(100,)
# $ /10GS/frames_interaction_energy: Shape-(100,)
# $ /10GS/frames_rmsd_ligand: Shape-(100,)
# $ /10GS/molecules_begin_atom_index: Shape-(3,)
# $ /10GS/trajectory_coordinates: Shape-(100, 6593, 3)