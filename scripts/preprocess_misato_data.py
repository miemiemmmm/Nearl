...
table = pd.read_csv(
    "/MieT5/Nearl/data/PDBBind_general_v2020.csv", delimiter=",", header=0
)
...
with io.hdffile(qmfile) as f1:
    count = 0
    for pdbcode in f1:
        if pdbcode.lower() not in pdblist and pdbcode.lower() not in SUPERSEDES:
            count += 1
...
with io.hdffile(mdfile) as f1:
    with open(listtocheck) as file_train:
    ...
    for pdbcode in thelist:
        ...
        if source == "lig":
            ...
            end_idx = atomnr if (seg_idx + 1) > len(bgatoms) else bgatoms[seg_idx + 1]
            ...
            print(
                f"{c} :: Not Found {source} :: MOL selection",
                traj.top.select(":MOL"),
                qm_atomnr,
                "-> Diff ",
                seg_diffs,
