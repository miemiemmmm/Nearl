...
import nearl
from nearl import _tempfolder, features
...
def parallelize_traj(traj_list, focused_res):
    ...
    c = 0
    # print(dir(nearl))
    nearl.CONFIG["tempfolder"] = (
        f"/home/yzhang/Documents/tests/tempfolder_mlproj/single_{focused_res.replace(':', '')}"
    )
    nearl.update_config()
    ...
    for complex_idx, complex_file in enumerate(traj_list):
        print(
            f"Processing {complex_file} ({complex_idx + 1}/{len(traj_list)}, last complex took {(time.perf_counter() - st_this):.2f} seconds)"
        )
        st_this = time.perf_counter()
            ...
            for atomidx in residues_CAs:
                ...
                if len(ret_array) == 0:
                    ...
                    ret_array = np.concatenate(
                        [ret_array, np.array(repr_traji)], axis=0
                    )

        if len(ret_array) // 100 > 10 or complex_idx == len(traj_list) - 1:
            tempfilename = os.path.join(
                _tempfolder, f"{focused_res.strip(':')}_{os.getpid()}.npy"
            )
...
if __name__ == "__main__":
    ...
    if do_test:
        ...
        results = [result1]
        print(
            f"# Task finished, time elapsed: {(time.perf_counter() - st) / 60:.2f} min"
        )
    else:
            ...
            with performance_report(filename="dask-report.html"):
                ...
                for trajlist in split_groups:
                    for residue in FOCUSED_RESIDUES:
                        c += 1
                        print(
                            f"Task {c} | focused residue: {residue} | trajectories: {trajlist}"
                        )
                        tasks.append(
                            dask.delayed(parallelize_traj)(trajlist, f":{residue}")
                        )

                # tasks = [dask.delayed(parallelize_traj)(traj_list) for traj_list in split_groups]
                print(
                    f"# Task set contains {len(tasks)} jobs; Jobs are generated and ready to run"
                )
                st = time.perf_counter()
