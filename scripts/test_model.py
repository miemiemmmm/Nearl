...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train the models used in the FEater paper. "
    )
    ...
    parser.add_argument(
        "-p", "--pretrained", type=str, required=True, help="Pretrained model path"
    )
    parser.add_argument(
        "-m",
        "--meta-information",
        type=str,
        required=True,
        help="The meta information file for the pretrained model",
    )
    parser.add_argument(
        "-o",
        "--output_folder",
        type=str,
        default="/tmp/",
        help="The output folder to store the model and performance data",
    )
    parser.add_argument(
        "-w",
        "--data-workers",
    ...
    # Miscellanenous
    parser.add_argument(
        "--test-number", type=int, default=100_000_000, help="Number of test samples"
    )
    parser.add_argument(
        "--test-robustness",
        type=int,
        default=0,
        help="Test the robustness of the model",
    )
    parser.add_argument(
...
def perform_testing_API(settings: dict, return_model=False, silent=False):
    ...
    test_data = nearl.io.dataset.Dataset(
        testfiles, feature_keys=DATATAGS, label_key=LABELTAG
    )

    model = nearl.utils.get_model(
        MODEL_TYPE,
        meta_information["input_channels"],
        meta_information["output_dimension"],
        meta_information["dimensions"],
    )
    ...
    if meta_information["pretrained"] and len(meta_information["pretrained"]) > 0:
        ...
        raise ValueError(
            f"Unexpected pretrained model {meta_information['pretrained']}"
        )
    if USECUDA:
        model.cuda()

    if meta_information["loss_function"] not in LOSS_FUNCTIONS:
        raise ValueError(
            f"Loss function {meta_information['loss_function']} is not supported."
        )
    else:
        criterion = LOSS_FUNCTIONS[meta_information["loss_function"]]()

    ret_pred, targets, ret_loss = nearl.utils.test_model(
        model, test_data, criterion, TESTNUMBER, BATCH_SIZE, USECUDA, WORKER_NR
    )
