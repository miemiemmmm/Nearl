...
def get_model(model_type: str, input_dim: int, output_dim: int, box_size, **kwargs):
    ...
    if model_type == "voxnet":  # Model done
        ...
        return nearl.models.model_deeprank.DeepRankNetwork(
            channel_nr, output_dim, box_size
        )

    elif model_type == "gnina2017":
        import nearl.models.model_gnina

        return nearl.models.model_gnina.GninaNetwork2017(
            channel_nr, output_dim, box_size
        )
        ...
        return nearl.models.model_gnina.GninaNetwork2018(
            channel_nr, output_dim, box_size
        )
        ...
        return nearl.models.model_pafnucy.PafnucyNetwork(
            channel_nr, output_dim, box_size, **kwargs
        )
...
def test_model(
    model, dataset, criterion, test_number, batch_size, use_cuda=1, process_nr=24
):
    test_loss = 0
    count = 0
    tested_sample_nr = 0
    ...
    with torch.no_grad():
        model.eval()
        for data, target in dataset.mini_batches(
            batch_size=batch_size, process_nr=process_nr
        ):
...
def perform_training(training_settings: dict):
    ...
    training_data = Dataset(
        trainingfiles, dimensions, feature_keys=datatags, label_key=labeltag
    )
    test_data = Dataset(
        testfiles, dimensions, feature_keys=datatags, label_key=labeltag
    )
    ...
    if training_settings["optimizer"] == "adam":
        optimizer = OPTIMIZERS[training_settings["optimizer"]](
            model.parameters(), lr=training_settings["lr_init"], **adam_kwargs
        )
    else:
        raise ValueError(f"Optimizer {training_settings['optimizer']} is not supported")
    ...
    for epoch in range(EPOCH_NR):
        ...
        for batch_idx, batch in enumerate(
            training_data.mini_batches(batch_size=BATCH_SIZE, process_nr=WORKER_NR)
        ):
            ...
            if (batch_idx + 1) % (batch_nr // 5) == 0:
                loss_on_train, metric_on_train = test_model(
                    model,
                    training_data,
                    criterion,
                ...
                loss_on_test, metric_on_test = test_model(
                    model,
                    test_data,
                    criterion,
                    training_settings["test_number"],
                ...
                if output_dims == 1:
                    ...
                    # Accuracy
                    jobmsg += (
                        f"Accuracy: {metric_on_test:>6.4f}/{metric_on_train:<6.4f}; "
                    )
                else:
                ...
                if (training_settings["verbose"] > 0) or (
                    ...
                    tensorboard_writer.add_scalar(
                        "Loss/Train", loss_on_train, epoch * batch_nr + batch_idx
                    )
                    tensorboard_writer.add_scalar(
                        "Loss/Test", loss_on_test, epoch * batch_nr + batch_idx
                    )
                    if output_dims == 1:
                        # RMSE
                        tensorboard_writer.add_scalar(
                            "RMSE/Train", metric_on_train, epoch * batch_nr + batch_idx
                        )
                        tensorboard_writer.add_scalar(
                            "RMSE/Test", metric_on_test, epoch * batch_nr + batch_idx
                        )
                    elif output_dims > 1:
                        # Accuracy
                        tensorboard_writer.add_scalar(
                    ...
                    inferred, gound_truth = test_samples(
                        model,
                        test_data,
                        criterion,
                        training_settings["test_number"],
                    ...
                    tensorboard_writer.add_figure(
                        "Scatter/Test", scatter_fig, epoch * batch_nr + batch_idx
                    )
                    ...
                    plt.scatter(gound_truth.cpu().numpy(), inferred.cpu().numpy())
                    tensorboard_writer.add_figure(
                        "Scatter/Train", scatter_fig, epoch * batch_nr + batch_idx
                    )
        ...
        loss_on_train, accuracy_on_train = test_model(
            model,
            training_data,
            criterion,
            training_settings["test_number"],
        ...
        loss_on_test, accuracy_on_test = test_model(
            model,
            test_data,
            criterion,
            training_settings["test_number"],
...
if __name__ == "__main__":
    ...
    SETTINGS["dimensions"] = 32  # TODO: this has to be set by the H5 dataset
