...
class FeatureLabel(features.Feature):
    ...
    def before(self):
        ...
        atom_counterpart = self.traj.top.select(
            f"(({reformed_mask})<:8)&(!@H=)&!({reformed_mask})"
        )
        self.pdist, self.pdistinfo = utils.PairwiseDistance(
            self.traj, atom_select, atom_counterpart
        )
        ...
        if self.pdist.mean() > 8:
            log.warning(
                "The mean distance between the atom of interest and its counterpart is larger than 8 Angstrom. Please check the atom selection"
            )
        elif np.percentile(self.pdist, 85) > 12:
            log.warning(
                "The 85th percentile of the distance between the atom of interest and its counterpart is larger than 12 Angstrom. Please check the atom selection"
            )
    ...
    def featurize(self):
        ...
        print(
            f"Labeling the frame by score: {penalized}/{baseline_affinity}; Penalty 1:{panelty1}; Penalty 2:{panelty2};"
        )
        return penalized
...
def parallelize_traj(traj_list):
    ...
    for trajectory in traj_loader:
        ...
        feat.register_feature(
            features.PenaltyFeature(
                f"({mask})&(!@H=)", f"({mask}<:5)&(!@H=)&(!{mask})", ref=0
            )
        )
...
if __name__ == "__main__":
    ...
    topologies = [
        "/media/yzhang/MieT5/BetaPose_trajs/C209CsDJQucZ_job_008_END.pdb"
    ] * 10
    ...
    if production:
        ...
        with Client(
            ...
            tasks = [
                dask.delayed(parallelize_traj)(traj_pairs)
                for traj_pairs in split_groups
            ]
