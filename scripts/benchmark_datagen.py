...
def parser():
    ...
    parser.add_argument(
        "-f", "--trajfiles", type=str, required=True, help="Input trajectory file path."
    )
    parser.add_argument(
        "-t",
        "--feature-type",
        type=int,
        required=True,
        help="Feature type to benchmark.",
    )
    parser.add_argument(
        "-o",
        "--outputfile",
        type=str,
        default="/tmp/benchmarking_dataset.h5",
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="The span for each frame-slice.",
    )
    parser.add_argument(
        "-n",
        "--num-repeat",
        type=int,
        default=5,
        help="Number of repeat for the benchmark",
    )
    parser.add_argument(
        "--force", type=int, default=0, help="Force to remove the output file."
    )
...
def get_features(feattype):
    ...
    features["feat_prot"] = nearl.features.Mass(
        selection="!:MOL", outkey="protMassStatic", cutoff=2.5, sigma=1
    )
    features["feat_lig"] = nearl.features.Mass(
        selection=":MOL", outkey="ligMassStatic", cutoff=2.5, sigma=1
    )

    # print("Adding marching observers")
    features["mo_prot"] = nearl.features.MarchingObservers(
        selection="!:MOL",
        weight_type="mass",
        obs="mean_distance",
        agg="drift",
        outkey="protDistObsDrift",
        cutoff=1,
    )
    features["mo_lig"] = nearl.features.MarchingObservers(
        selection=":MOL",
        weight_type="mass",
        obs="mean_distance",
        agg="drift",
        outkey="ligDistObsDrift",
        cutoff=1,
    )

    # print("Adding probability density flow")
    features["pdf_prot"] = nearl.features.DensityFlow(
        selection="!:MOL",
        weight_type="mass",
        outkey="protMassPropDrift",
        agg="drift",
        cutoff=2.5,
        sigma=1.0,
    )
    features["pdf_lig"] = nearl.features.DensityFlow(
        selection=":MOL",
        weight_type="mass",
        outkey="ligMassPropDrift",
        agg="drift",
        cutoff=2.5,
        sigma=1.0,
    )
    ...
    if feattype == 1:
        features["feat_static"] = nearl.features.AtomType(
            selection="!:MOL", focus_element=6, outkey="feat_static", cutoff=4
        )
        outkey = "feat_static"
    elif feattype == 2:
        features["feat_mo"] = nearl.features.MarchingObservers(
            selection="!:MOL",
        ...
        features["feat_pdf"] = nearl.features.DensityFlow(
            selection="!:MOL",
            weight_type="atom_type",
            element_type=6,
            outkey="feat_pdf",
...
def commandline_interface(args, outputdim):
    ...
    with h5.File(outputfile, "r") as f:
        ...
        time_elapsed = ed - st
        print(
            f"Dimension: {outputdim}; Featurization throughput: {data_size / time_elapsed:6.3f}; Time elapse: {time_elapsed:6.3f}; Feature number: {data_size:3d};"
        )
