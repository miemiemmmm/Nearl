...
def transform_entry(hdffile, tagname, idx, map1, map2, transform_settings):
    ...
    with io.hdffile(hdffile, "r") as f:
        ...
        transform_2d = np.zeros(
            (int(np.sqrt(len(map2))), int(np.sqrt(len(map2)))), dtype=np.float32
        )
...
def make_hdf(inputhdf: str, outputhdf: str, interp_settings: dict):
            ...
            if isinstance(hdf[datatag], h5.Dataset):
                if hdf[datatag].shape.__len__() < 3:
                    ...
                    with h5.File(outputhdf, "a") as f:
                        srcdata = hdf[datatag]
                        dstdata = f.create_dataset(
                            datatag, srcdata.shape, dtype=srcdata.dtype
                        )
                        dstdata[...] = srcdata[...]
                else:
                    print(
                        f"Keeping the the dataset named {datatag} for further processing"
                    )
                    datasets_to_process.append(datatag)
            elif isinstance(hdf[datatag], h5.Group):
                print(
                    f"Directly copying the the group named {datatag} to the output hdf file"
                )
    ...
    print(
        f"Processing {len(datasets_to_process)} datasets from {inputhdf} to {outputhdf}"
    )
    ...
    print(
        f"3D Hilbert curve has {len(coord_3d)} points, 2D Hilbert curve has {len(coord_2d)} points"
    )
        ...
        for datatag in datasets_to_process:
            ...
            with io.hdffile(outputhdf, "a") as f:
                ...
                if "dimensions_3d" not in f["featurizer_parms"].keys():
                    ...
                    f["featurizer_parms"].create_dataset(
                        "dimensions_3d", data=old_dims, dtype=np.int64, maxshape=(3,)
                    )
                    f["featurizer_parms"].create_dataset(
                        "dimensions",
                        data=np.array([len_2d, len_2d], dtype=np.int64),
                        dtype=np.int64,
                        maxshape=(2,),
                    )
            ...
            for idx, batch in enumerate(batches):
                results = pool.starmap(
                    transform_entry,
                    [
                        (inputhdf, datatag, _idx, coord_3d, coord_2d, interp_settings)
                        for _idx in batch
                    ],
                )
                ...
                with io.hdffile(outputhdf, "a") as f:
                    ...
                    if interp_settings.get("compress_level", 0) > 0:
                        ...
                        extra_config["compression_opts"] = interp_settings[
                            "compress_level"
                        ]

                    utils.add_data_to_hdf(
                        f,
                        datatag,
                        image_buffer,
                        dtype=np.float32,
                        maxshape=(None, len_2d, len_2d),
                        chunks=(1, len_2d, len_2d),
                        **extra_config,
                    )
                ...
                st_batch = time.perf_counter()
                print(
                    f"Batch {idx:4d} / {len(batches):4d} ({len(batch):4d} entries) done in {(time_estimate) * 1000:6.2f} ms, Average speed: {time_estimate * 1000 / len(batch):6.2f} ms per entry"
                )
                print(
                    f">>>> Estimated time left: {time_estimate * (len(batches) - idx):6.2f} s"
                )
            pool.close()
            pool.join()
