    ...
    for key in slices:
        ...
        if key == "x":
            for y in range(dims[1]):
                for z in range(dims[2]):
                    box = o3d.geometry.TriangleMesh.create_box(
                        width=0.001, height=pixel_size, depth=pixel_size
                    )
                    box.translate(
                        np.asarray(
                            [
            ...
            for x in range(dims[0]):
                for z in range(dims[2]):
                    box = o3d.geometry.TriangleMesh.create_box(
                        width=pixel_size, height=0.001, depth=pixel_size
                    )
                    box.translate(
                        np.asarray(
                            [
            ...
            for x in range(dims[0]):
                for y in range(dims[1]):
                    box = o3d.geometry.TriangleMesh.create_box(
                        width=pixel_size, height=pixel_size, depth=0.001
                    )
                    box.translate(
                        np.asarray(
                            [
...
def main_render(inputfile: str, index: int, args):
    ...
    print(
        f"The sum of the voxel is {np.sum(voxeli):6.3f}: Max {np.max(voxeli):6.3f}, Min {np.min(voxeli):6.3f}"
    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="The index of the molecule to be viewed",
    )
    parser.add_argument(
        "-p",
        "--percentile",
        type=int,
        default=90,
        help="The percentile of the voxel to be viewed",
    )
    parser.add_argument(
        "-c", "--cmap", type=str, default="inferno", help="The colormap to be used"
    )
    parser.add_argument(
        "-t", "--tagname", type=str, default="voxel", help="The tag name of the voxel"
    )
    parser.add_argument(
        "-hide", "--hide", default=1, type=int, help="Hide the zero voxels. Default: 1."
    )
    parser.add_argument(
        "-m",
        "--markcenter",
        default=1,
        type=int,
        help="Mark the center of the voxel (Marked by a green sphere). Default: 1",
    )
    parser.add_argument(
        "-r",
        "--reference",
        type=str,
        default=None,
        help="The reference file to be compared with",
    )
    parser.add_argument(
        "-b",
        "--boundingbox",
        type=int,
        default=1,
        help="Add bounding box to the voxel. Default: 1",
    )
