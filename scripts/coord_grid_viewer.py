...
def boxinfo_to_coord(theboxinfo):
    dim1 = np.linspace(
        theboxinfo[0] - theboxinfo[3] / 2,
        theboxinfo[0] + theboxinfo[3] / 2,
        int(theboxinfo[6]),
    )
    dim2 = np.linspace(
        theboxinfo[1] - theboxinfo[4] / 2,
        theboxinfo[1] + theboxinfo[4] / 2,
        int(theboxinfo[7]),
    )
    dim3 = np.linspace(
        theboxinfo[2] - theboxinfo[5] / 2,
        theboxinfo[2] + theboxinfo[5] / 2,
        int(theboxinfo[8]),
    )
...
idx = 17
with data_io.hdf_operator(
    "/media/yzhang/MieT5/Nearl/data/trainingdata/test_3d_data.h5", "r"
) as f:
    #   f.draw_structure()
