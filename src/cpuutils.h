...
template <typename T> T gaussian_map(T dist, double mu = 0.0, double sigma = 1.0) {
...
template <typename T> T mean(const T *Arr, const int N) {
  T thesum = sum(Arr, N);
  return thesum / N;
}


template <typename T> T standard_deviation(const T *x, const int n) {
  T mean_val = mean(x, n);
  T sum = 0;
  for (int i = 0; i < n; ++i) {
...
template <typename T> T variance(const T *x, const int n) {
  T _mean = mean(x, n);
  T sum = 0;
  for (int i = 0; i < n; ++i) {
...
template <typename T> T median(const T *x, const int n) {
  T *y = new T[n];
  for (int i = 0; i < n; ++i) {
    y[i] = x[i];
  }
...
template <typename T>
std::vector<std::vector<double>> CosineSimilarityBatch(const std::vector<std::vector<T>> &vecs1,
                                                       const std::vector<std::vector<T>> &vecs2) {
...
double EuclideanSimilarity(const std::vector<T> &vec1, const std::vector<T> &vec2) {
  ...
  for (size_t i = 0; i < vec1.size(); ++i) {
    sum += (static_cast<double>(vec1[i]) - static_cast<double>(vec2[i])) *
           (static_cast<double>(vec1[i]) - static_cast<double>(vec2[i]));
  }
  return sqrt(sum);
}
...
EuclideanSimilarityBatch(const std::vector<std::vector<T>> &vecs1,
                         const std::vector<std::vector<T>> &vecs2) {
  unsigned int len1 = vecs1.size();
  unsigned int len2 = vecs2.size();
...
template <typename T>
std::vector<std::vector<double>>
ManhattanSimilarityBatch(const std::vector<std::vector<T>> &vecs1,
                         const std::vector<std::vector<T>> &vecs2) {
  unsigned int len1 = vecs1.size();
  unsigned int len2 = vecs2.size();
  std::vector<std::vector<double>> similarities(len1, std::vector<double>(len2, 0.0));
...
double JaccardSimilarity(const std::vector<T> &vec1, const std::vector<T> &vec2) {
  ...
  std::set_intersection(sorted_vec1.begin(), sorted_vec1.end(), sorted_vec2.begin(),
                        sorted_vec2.end(), std::back_inserter(intersection));
...
template <typename T>
std::vector<std::vector<double>> JaccardSimilarityBatch(const std::vector<std::vector<T>> &vecs1,
                                                        const std::vector<std::vector<T>> &vecs2) {
  unsigned int len1 = vecs1.size();
  unsigned int len2 = vecs2.size();
  std::vector<std::vector<double>> similarities(len1, std::vector<double>(len2, 0.0));
...
template <typename T> bool almost_equal(const T &a, const T &b) {
 ...
 * in the gpuutils.cuh
 */
template <typename T> double information_entropy(const T *Arr, const int N) {
  if (N <= 1) {
    return 0.0f;
  }
...
inline void translate_coord(float *coord, const int atom_nr, const int *dims,
