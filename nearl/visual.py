...
element_color_map = {
    ...
    15: [
        ...
        0,
        0,
    ],  # TODO: Need to correct the data in the hdf file. Round to closest interger not the floor
    16: [1, 0, 0],
    32: [1, 1, 0],
    31: [1, 0.6, 0.4],
    # METALS
    ...
    for x in range(dims[0]):
        for y in range(dims[1]):
            for z in range(dims[2]):
                ...
                trans = np.asarray(
                    [x * scale_factor[0], y * scale_factor[1], z * scale_factor[2]],
                    dtype=np.float64,
                )
                box.translate(trans)
    ...
    for key, value in slices.items():
        ...
        if key == "x":
            for y in range(dims[1]):
                for z in range(dims[2]):
                    box = o3d.geometry.TriangleMesh.create_box(
                        width=0.001, height=0.45, depth=0.45
                    )
                    box.translate(
                        np.asarray(
                            [
            ...
            for x in range(dims[0]):
                for z in range(dims[2]):
                    box = o3d.geometry.TriangleMesh.create_box(
                        width=0.45, height=0.001, depth=0.45
                    )
                    box.translate(
                        np.asarray(
                            [
            ...
            for x in range(dims[0]):
                for y in range(dims[1]):
                    box = o3d.geometry.TriangleMesh.create_box(
                        width=0.45, height=0.45, depth=0.001
                    )
                    box.translate(
                        np.asarray(
                            [
...
def add_bounding_box(dims):
    ...
    ret = []
    for line in lines:
        cylinder = view_obj.create_cylinder(
            boxpoints[line[0]], boxpoints[line[1]], radius=0.1, color=[0, 0, 1]
        )
        ret.append(cylinder)
    print(len(ret))
    for geo in ret:
...
def render_slices(inputfile: str, index: int, args):
    ...
    geoms_voxel = get_geo_slices(
        voxeli, slices=slice_dict, cmap=args.cmap, scale_factor=scale_factor
    )
...
def parse_view_slice():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="The index of the molecule to be viewed, default: 0",
    )
    parser.add_argument(
        "-t",
        "--tagname",
        type=str,
        default="voxel",
        help="The tag name of the voxel, default: voxel",
    )
    parser.add_argument(
        "-c",
        "--cmap",
        type=str,
        default="inferno",
        help="The colormap to be used, default: inferno",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Whether or not to add a bounding box of the voxel. Default: 1",
    )
    parser.add_argument(
        "-s",
        "--saveply",
        type=str,
        default=None,
        help="The output file to save the ply object",
    )
    parser.add_argument(
        "-x",
        "--x",
        type=int,
        default=-1,
        help="The x slice to be viewed, default: -1 (hide)",
    )
    parser.add_argument(
        "-y", "--y", type=int, default=15, help="The y slice to be viewed, default: 15"
    )
    parser.add_argument(
        "-z", "--z", type=int, default=15, help="The z slice to be viewed, default: 15"
    )
...
def parse_view_voxel():
    ...
    parser.add_argument(
        ...
        type=int,
        default=95,
        help="The percentile to be used for the cutoff, default: 95",
    )
    parser.add_argument(
        "--hide",
        type=int,
        default=1,
        help="Whether or not to hide the zero voxels, default: 1",
    )
    parser.add_argument(
        "-b",
        "--boundingbox",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The output file to save the obj object, default: None",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The output file to save the image upon closing the 3D viewer, default: None",
    )  # TODO: Implement this later
...
def render_voxels(inputfile: str, index: int, args):
    ...
    with h5py.File(inputfile, "r") as hdf:
        ...
        # Get dimensions metadata from the hdf file
        dim = hdf["featurizer_parms"]["dimensions"][()]
        dims = np.array(dim) if hasattr(dim, "__len__") else np.array([dim, dim, dim])
