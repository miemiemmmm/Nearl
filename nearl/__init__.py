...
def _ensure_file_handler():
    ...
    if _file_handler is None:
        logpath = os.path.join(
            os.path.abspath(config.tempfolder()), f"nearl.{os.getpid()}.log"
        )
...
def _log(level, arg, kwarg):
    ...
    if override_stream is not None:
        ...
        out = (
            f"{color}{message}\033[0m"
            if color and override_stream.isatty()
            else message
        )
        __builtinprint(out, file=override_stream, **kwarg)
        _file_handler.handle(
            _package_logger.makeRecord(
                _package_logger.name, level, __file__, 0, message, None, None
            )
        )
    else:
...
def summary():
    log(
        f"Summary: {'Clear' if config.clear() else 'Keep'} temporary files; {'Verbose' if config.verbose() else 'Silent'} mode; {'Using GPU' if config.usegpu() else 'Using CPU only'}"
    )
    if (not os.path.exists(config.tempfolder())) or (
        not os.path.isdir(config.tempfolder())
    ):
        raise OSError(
            "The temporary folder (tempfolder) does not exist; Please check the configuration file"
        )
...
def draw_call_stack():
    ...
    for frame_info in __call_stack():
        log(
            f"Function: {frame_info.function:<20s} | Line: {frame_info.lineno:<5d} from File: {frame_info.filename:40}"
        )
