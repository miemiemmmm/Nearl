...
class GninaNetwork2017(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(conv)):
            if i == 0:
                conv_layers["conv3d_1"] = nn.Conv3d(
                    input_channels, conv[i], kernel_size=3, padding=1
                )
            else:
                conv_layers[f"conv3d_{i + 1}"] = nn.Conv3d(
                    conv[i - 1], conv[i], kernel_size=3, padding=1
                )
...
class GninaNetwork2018(nn.Module):
    ...
    def __init__(self, input_channels, output_dimension, input_shape):
        ...
        conv_layers["conv1"] = nn.Conv3d(
            input_channels, 32, kernel_size=3, padding=1, stride=1
        )
...
class GninaNetworkDense(nn.Module):
    def __init__(
        self, dims, num_dense_blocks=3, num_dense_filters=16, num_dense_convs=4
    ):
        super().__init__()
        ...
        dense_block = DenseBlock(
            in_feats, num_dense_blocks - 1, num_dense_convs, num_dense_filters
        )
        out_feats = num_dense_convs * num_dense_filters + in_feats
        ...
        self.add_module("data_enc_level2_global_pool", global_pool)
