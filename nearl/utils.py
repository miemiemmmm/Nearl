...
def get_hash(theinput="", mode="md5"):
    ...
    if mode == "md5":
        ...
        print(
            "Warning: Not found a valid hash function (md5, sha1, sha256, sha512), use md5 by default."
        )
        hash_func = hashlib.md5
    if isinstance(theinput, str) and len(theinput) > 0:
        ...
        log.warning(
            "Not a valid input, should be (string, tuple, list, np.ndarray, bytes). Using time.perf_counter() by default."
        )
        return hash_func(bytes(time.perf_counter().__str__(), "utf-8")).hexdigest()
...
def get_protein_mask(_structure):
    ...
    if isinstance(_structure, str) and os.path.isfile(_structure):
        ...
        raise ValueError(
            "Invalid input structure (must be a file path or a pytraj trajectory object)"
        )
...
def filter_points_within_bounding_box(
    thearr, grid_center, grid_length, return_coord=False
):
    """
...
def generate_gridcoords(thecenter, thelengths, thedims):
    ...
    thecoord = np.column_stack(
        [thegrid[0].ravel(), thegrid[1].ravel(), thegrid[2].ravel()]
    )
...
def get_pdb_seq(pdbcode):
    ...
    chainids = [
        i[11] for i in pdbstr.split("\n") if re.search(r"SEQRES.*[A-Z].*[0-9]", i)
    ]
    chainid = chainids[0]
    title = " ".join(
        [
            i[19:]
            for i in pdbstr.split("\n")
...
def conflict_factor(pdbfile, ligname, cutoff=5):
    ...
    for i, coord in enumerate(coords):
        ...
        vdw_pairs = (
            np.array(
                [
                    constants.VDWRADII[i.atomic_number]
                    for i in np.array([*traj.top.atoms])[otheratoms]
                ]
            )
            + constants.VDWRADII[thisatom]
        )
        ...
        if np.count_nonzero(st) > 0:
            ...
            for part in partatoms:
                dist = distance_matrix(
                    [traj.xyz[0][part.index]], [traj.xyz[0][thisatom.index]]
                )
                print(
                    f"Found clash between: {thisatom.name}({thisatom.index}) and {part.name}({part.index}); Distance: {dist.squeeze().round(3)}"
                )

    factor = 1 - ((cclash / 2) / ((ccontact / 2) / atomnr))
    print(
        f"Clashing factor: {round(factor, 3)}; Atom selected: {atomnr}; Contact number: {ccontact}; Clash number: {cclash}"
    )
    return factor
        ...
        try:
            ...
            if previous_res is not None:
                ...
                if f"{previous_res.name}{this_res.name}" != key:
                    ...
                    top_res1 = traj.top[
                        previous_res.first_atom_index : previous_res.last_atom_index
                    ]
                    crd_res1 = coord[
                        previous_res.first_atom_index : previous_res.last_atom_index, :
                    ]
                    top_res2 = traj.top[
                        this_res.first_atom_index : this_res.last_atom_index
                    ]
                    crd_res2 = coord[
                        this_res.first_atom_index : this_res.last_atom_index, :
                    ]
                    ...
                    if len(atomc) == 0 or len(atomn) == 0:
                        ...
                        print(
                            "Warning: More than one C or N atom found in the residue, meaning there are some problem with the input structure."
                        )
                        previous_res = this_res
                        continue
                    else:
                        peptide_bond_len = np.linalg.norm(
                            crd_res1[atomc[0]] - crd_res2[atomn[0]]
                        )
                        if peptide_bond_len > PIPTIDE_BOND_THRESHOLD:
                            previous_res = this_res
                            continue
                        theslice = np.s_[
                            previous_res.first_atom_index : this_res.last_atom_index
                        ]
...
def get_model(model_type: str, input_dim: int, output_dim: int, box_size, **kwargs):
    ...
    if model_type == "voxnet":
        ...
        return nearl.models.model_resnet3d.ResNet3D(
            channel_nr, output_dim, box_size, **kwargs
        )
