...
def traj_to_rdkit(traj, atomidx, frameidx):
    ...
    try:
        ...
        Chem.SanitizeMol(
            mol, Chem.SanitizeFlags.SANITIZE_ALL ^ Chem.SanitizeFlags.SANITIZE_ADJUSTHS
        )
        ...
        for atom in mol.GetAtoms():
            if np.isnan(atom.GetDoubleProp("_GasteigerCharge")):
                ...
                if atomsymbol in DEFAULT_PARTIAL_CHARGE:
                    log.warning(
                        f"Found Nan in rdkit molecule; Setting the atom {atomsymbol} to its default partial charge {DEFAULT_PARTIAL_CHARGE[atomsymbol]}"
                    )
                    atom.SetDoubleProp(
                        "_GasteigerCharge", DEFAULT_PARTIAL_CHARGE[atomsymbol]
                    )
                else:
                    atom.SetDoubleProp("_GasteigerCharge", 0.0)
                    log.warning(
                        f"Found NaN in rdkit molecule; Atom {atomsymbol} not found in default preset. _GasteigerCharge set to 0.0"
                    )
        if True in np.isnan(
            [atom.GetDoubleProp("_GasteigerCharge") for atom in mol.GetAtoms()]
        ):
            log.warning(
                "Still found nan in the charge",
                [atom.GetDoubleProp("_GasteigerCharge") for atom in mol.GetAtoms()],
            )
        return mol
...
def DACbytraj(traj, frameidx, themask, **kwargs):
    ...
    if len(selection) == 0:
        log.warning("No atom in the selected mask. Skipping it.")
        return np.array([]), np.array([])
    if (">" in themask) or ("<" in themask):
        log.warning(
            "Detected distance based mash. Please make sure that the reference is set to the trajectory, otherwise, all of the atoms will be processed"
        )
    ...
    if (tmp_traj.top.n_atoms == traj.top.n_atoms) and config.verbose():
        log.warning(
            "All atoms are kept after applying the mask. Please make sure if this is wanted."
        )
...
def Chargebytraj(traj, frameidx, atomidx):
    ...
    if len(atomidx) == 0:
        print(
            f"{Chargebytraj.__name__:15s}: No atom in the selected mask. Skipping it."
        )
        return np.array([]), np.array([])
    atomnr = len(atomidx)
    ...
    if not np.isclose(atomnr, mol.GetNumAtoms()):
        ...
        charges = np.zeros(atomnr)
        print(
            f"Warning: The atom number of given atom index and PDB does not match. {atomnr} vs {mol.GetNumAtoms()}"
        )
...
def atom_name_mapping(pdbstr):
    ...
    pdbstr = (
        pdbstr.replace("NA", "Na").replace("MG", "Mg").replace("ZN", "Zn")
    )  # .replace("CA", "Ca").replace("CO", "Co")
    pdbstr = (
        pdbstr.replace("FE", "Fe")
        .replace("CU", "Cu")
        .replace("NI", "Ni")
        .replace("MN", "Mn")
    )
...
def correct_mol_by_smiles(refmol2, prob_smiles):
    ...
    if mol1 is None:
        log.error(
            "Failed to process the smiles. Please check the validity of the smiles"
        )
        return None
    elif mol2 is None:
        log.error(
            "Failed to process the mol2 file. Please check the validity of the mol2 file"
        )
        return None
...
def sanitize_bond(mol_raw):
    ...
    for bond in mol_raw.GetBonds():
        ...
        if (bond_length > bond_length_expected) and (
            not np.isclose(bond_length, bond_length_expected, rtol=0.1)
        ):
            log.warning(
                f"Removing abnormal bond {bond_rep} lengthed {bond_length:.2f}/{bond_length_expected} angstorm, formed by {begin_atom_idx}@{elem1} - {end_atom_idx}@{elem2}"
            )
