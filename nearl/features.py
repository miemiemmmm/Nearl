...
class Feature:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.translate == "center":
            log(
                f"{self.classname}: Will translate the coordinates of focused substructure to center of the bounding box. "
            )
        elif self.translate == "origin":
            log(
                f"{self.classname}: Will translate the coordinates of focused substructure to align the origin of the bounding box. "
            )
        else:
            logger.warning(
                f"{self.classname}: The translate parameter is not recognized. "
            )
    ...
    def dims(self, value):
        if isinstance(value, (int, float, np.int64, np.float64, np.int32, np.float32)):
            ...
            logger.warning(
                f"{self.classname} The dimension should be a number, list, tuple or a numpy array, not {type(value)}"
            )
            self.__dims = None
    ...
    def spacing(self, value):
        if isinstance(value, (int, float, np.int64, np.float64, np.int32, np.float32)):
            ...
            logger.warning(
                f"{self.classname}: The spacing is not a valid number, setting to None"
            )
            self.__spacing = None
        if self.__dims is not None and self.__spacing is not None:
    ...
    def cache(self, trajectory):
        ...
        if self.selection is None:
            ...
            if len(self.selection) != len(self.atomic_numbers):
                logger.warning(
                    f"{self.classname}: The length of selection array does not match the number of atoms {trajectory.n_atoms} in the topology. "
                )
    ...
    def query(self, topology, frame_coords, focal_point):
        ...
        if self.center is None or self.lengths is None or self.padding is None:
            ...
            if np.count_nonzero(mask) == 0:
                logger.warning(
                    f"{self}: Did not find atoms in the bounding box near the focal point {np.round(focal_point, 1)}"
                )
    ...
    def run(self, coords, weights):
        ...
        if len(coords) == 0:
            logger.warning(
                f"{self.classname}: The coordinates from query method are empty, returning an all-zero array"
            )
    ...
    def dump(self, result):
        ...
        """
        logger.debug(
            f"{self.classname}: Dumping the result to the HDF5 file {self.outfile} with the key {self.outkey}"
        )
        if (
            ("outfile" in dir(self))
            and ("outkey" in dir(self))
            and (len(self.outfile) > 0)
        ):
            if np.isnan(result).sum() > 0:
                logger.warning(
                    f"{self.classname}: Found {np.isnan(result).sum()} NaN values in the result array"
                )
                result = np.nan_to_num(result).astype(
                    result.dtype
                )  # Replace NaN with 0
...
class Mass(Feature):
    ...
    def cache(self, trajectory):
        ...
        self.cached_array = np.array(
            [constants.ATOMICMASS[i] for i in self.atomic_numbers], dtype=np.float32
        )
...
class Radius(Feature):
    def cache(self, trajectory):
        super().cache(trajectory)
        self.cached_array = np.array(
            [constants.ATOMICRADIUS[i] for i in self.atomic_numbers], dtype=np.float32
        )
...
class Aromaticity(Feature):
    ...
    def cache(self, trajectory):
        ...
        if self.reverse:
            self.cached_array = np.array(
                [1 if i == 0 else 0 for i in atoms_aromatic], dtype=np.float32
            )
        else:
...
class Ring(Feature):
    ...
    def cache(self, trajectory):
        ...
        if self.reverse:
            self.cached_array = np.array(
                [1 if i == 0 else 0 for i in atoms_in_ring], dtype=np.float32
            )
        else:
...
class Selection(Feature):
    ...
    def query(self, topology, frame_coords, focal_point):
        ...
        idx_inbox, coord_inbox = super().query(topology, frame_coords, focal_point)
        weights = self.cached_array[idx_inbox]
        assert len(weights) == len(coord_inbox), (
            f"{self.classname}: The length of weights {len(weights)} does not match the length of coordinates {len(coord_inbox)}"
        )
        return coord_inbox, weights
...
class Backbone(Feature):
    ...
    def cache(self, trajectory):
        super().cache(trajectory)
        backboneness = [
            1 if i.name in ["C", "O", "CA", "HA", "N", "HN"] else 0
            for i in trajectory.top.atoms
        ]
        if self.reverse:
            self.cached_array = np.array(
...
class AtomType(Feature):
    ...
    def __str__(self):
        ret_str = (
            f"{self.classname} <focus:{constants.ATOMICNR2SYMBOL[self.focus_element]}>"
        )
        return ret_str
...
class PartialCharge(Feature):
    ...
    def __init__(
        self, charge_type="topology", charge_parm=None, keep_sign="both", **kwargs
    ):
        super().__init__(**kwargs)
        ...
        if charge_type not in ["topology", "manual", "chargefw", "external"]:
            raise ValueError(
                f"{self.classname}: The charge type should be either 'topology', 'manual', 'chargefw' or 'external' rather than {charge_type}"
            )
        if keep_sign not in ["positive", "negative", "both", "p", "n", "b"]:
            raise ValueError(
                f"{self.classname}: The keep_sign parameter should be either positive/p, negative/n or both/b rather than {keep_sign}"
            )
    ...
    def cache(self, trajectory):
        ...
        if self.charge_type == "topology":
            if np.allclose(trajectory.top.charge, 0):
                raise ValueError(
                    f"{self.classname}: The charge values in the topology are all zero meaning the topology of the trajectory does not contain necessary charge information. "
                )
            else:
            ...
            assert len(self.charge_parm) == trajectory.top.n_atoms, (
                "The number of charge values does not match the number of atoms in the trajectory"
            )
            ...
            try:
                ...
                raise ImportError(
                    f"{self.classname}: The ChargeFW2 is not installed. Check the https://github.com/sb-ncbr/ChargeFW2 for more information. "
                ) from e
            charges = None
            charge_values = None
            mol = None
            with tempfile.NamedTemporaryFile(suffix=".pdb") as f:
                pt.write_traj(
                    f.name, trajectory, format="pdb", frame_indices=[0], overwrite=True
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"{self.classname}: Failed to load the molecule because of the following error:\n{e}"
                    )
                ...
                try:
                    ...
                    if filename not in charges:
                        ...
                        if len(charge_values) != trajectory.n_atoms:
                            raise ValueError(
                                f"{self.classname}: The number of charge values does not match the number of atoms in the trajectory"
                            )
                        else:
                    ...
                    logger.warning(
                        f"{self.classname}: Default charge method {self.charge_type} failed due to the following error:\n{e}"
                    )
                    for method, parmfile in cfw.get_suitable_methods(mol):
                        ...
                        if len(parmfile) > 0:
                            log(
                                f"{self.classname}: Trying alternative charge methods {method} with {parmfile} parameter"
                            )
                        else:
                            log(
                                f"{self.classname}: Trying alternative charge methods {method} without parameter file"
                            )
                        try:
                            ...
                            if filename not in charges:
                                ...
                                charge_values = np.array(
                                    charges[filename], dtype=np.float32
                                )
                                if len(charge_values) != trajectory.n_atoms:
                                    continue
                                else:
                                    log(
                                        f"{self.classname}: Finished the charge computation with {method} method"
                                        + (
                                            " without parameter file"
                                            if len(parmfile) == 0
                                            else " with parameter file"
                                        )
                                    )
                                    self.cached_array = charge_values
                                    break
                            ...
                            logger.warning(
                                f"{self.classname}: Failed to calculate molecular charge (Alternative charge types) because of the following error:\n {e}"
                            )
                            continue

            if charges is None or charge_values is None:
                raise RuntimeError(
                    f"{self.classname}: The charge computation failed for all attempted ChargeFW2 methods."
                )

        assert self.cached_array is not None, (
            f"{self.classname}: The charge values are not set. Please check the charge type and parameters. "
        )
        assert len(self.cached_array) == trajectory.n_atoms, (
            f"{self.classname}: The number of charge values does not match the number of atoms in the trajectory. "
        )
...
class Electronegativity(Feature):
    ...
    def cache(self, trajectory):
        super().cache(trajectory)
        self.cached_array = np.array(
            [constants.ELECNEG[i] for i in self.atomic_numbers], dtype=np.float32
        )
...
def cache_properties(trajectory, property_type, **kwargs):
    ...
    if property_type == 1:
        ...
        # Mass
        cached_arr = np.array(
            [constants.ATOMICMASS[i] for i in atom_numbers], dtype=np.float32
        )
        ...
        # Electronegativity
        cached_arr = np.array(
            [constants.ELECNEG[i] for i in atom_numbers], dtype=np.float32
        )
        ...
        # Sidechainness
        sidechainness = [
            0 if i.name in ["C", "O", "CA", "HA", "N", "HN"] else 1 for i in atoms
        ]
        ...
        cached_arr = np.array(
            [1 if i == focus_element else 0 for i in atom_numbers], dtype=np.float32
        )
...
class DynamicFeature(Feature):
    ...
    def weight_type(self):
        ...
        """
        if self._weight_type in SUPPORTED_FEATURES:
            return SUPPORTED_FEATURES[self._weight_type]
        else:
            raise ValueError(
                f"The weight type is not recognized {self._weight_type}; Available types are {SUPPORTED_FEATURES.keys()}"
            )
    ...
    def cache(self, trajectory):
        ...
        self.cached_array = cache_properties(
    ...
    def run(self, frames, weights):
        ...
        raise NotImplementedError(
            "The run function should be implemented in the child class"
        )
...
class MarchingObservers(DynamicFeature):
    ...
    def obs(self):
        ...
        """
        if self._obs_type in SUPPORTED_OBSERVATION:
            return SUPPORTED_OBSERVATION[self._obs_type]
        else:
            raise ValueError(
                f"The observation type is not recognized {self._obs_type}; Available types are {SUPPORTED_OBSERVATION.keys()}"
            )
    ...
    **kwargs : dict
    ...
    def __init__(self, baseline_map, colname="pK", **kwargs):
        ...
        if self.colname not in self.baseline_table.columns:
            raise ValueError(
                f"The column name {self.colname} is not found in the baseline table"
            )
    ...
    def cache(self, trajectory):
        ...
        if config.verbose():
            log(
                f"{self.classname}: Affinity value of {trajectory.identity} is {self.base_value}"
            )
...
class LabelStepping(LabelAffinity):
    ...
    def query(self, *args):
        assert self.base_value is not None, (
            "The base value should be set before the query"
        )
...
class LabelRMSD(LabelAffinity):
    def __init__(self, base_value=0, outshape=(None,), **kwargs):
        if "selection" not in kwargs:
    ...
    def cache(self, trajectory):
        ...
        self.cached_array = pt.rmsd_nofit(
            trajectory, mask=self.selection, ref=self.refframe
        )
        self.cached_array = np.minimum(
            self.cached_array, RMSD_CUTOFF
        )  # Set a cutoff RMSD for limiting the z-score
    ...
    **kwargs : dict
    ...
    def cache(self, trajectory):
        ...
        for i in range(len(selected)):
            dist = np.linalg.norm(
                self.refframe.xyz[selected[i]] - self.refframe.xyz[backbone_cb], axis=1
            )
            self.selected_counterpart[i] = backbone_cb[np.argmin(dist)]

        self.cached_array = utils.compute_pcdt(
            trajectory,
            self.selected,
            self.selected_counterpart,
            ref=self.refframe,
            return_info=False,
        )
