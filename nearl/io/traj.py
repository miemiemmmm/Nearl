...
class Trajectory(pt.Trajectory):
    ...
    def __init__(self, traj_src=None, pdb_src=None, **kwarg):
        ...
        if config.verbose():
            log(
                f"{self.__class__.__name__}: Loading trajectory {traj_src} with topology {pdb_src}"
            )
            log(
                f"{self.__class__.__name__}: stride: {stride}; frame_indices: {frame_indices}; mask: {mask}"
            )
        ...
        if isinstance(traj_src, str) and isinstance(pdb_src, str):
            ...
            tmptraj = pt.load(
                traj_src, pdb_src, stride=stride, frame_indices=frame_indices, mask=mask
            )
            ...
            raise ValueError(
                f"Invalid input for traj source ({type(traj_src)}) and pdb_src ({type(pdb_src)})"
            )
        ...
        top = tmptraj.top
        xyz = tmptraj.xyz
        assert tmptraj.top.n_atoms == tmptraj.xyz.shape[1], (
            f"The number of atoms in the topology and the coordinates should be the same, rather than {tmptraj.top.n_atoms} and {tmptraj.xyz.shape[1]}"
        )

        # Set basic attributes for pytraj.Trajectory;
        super().__init__(
            xyz=xyz, top=top, velocity=tmptraj.velocities, force=tmptraj.forces
        )
        self._boxes = boxinfo
        self.time = timeinfo
    ...
    def add_dummy_points(self, coordinates, elements=None, frame_idx=0, outfile=""):
        ...
        if elements is None:
            ...
            assert len(elements) == len(coordinates), (
                "The length of elements should be the same as the coordinates"
            )
        ...
        for i, c in enumerate(coordinates):
            therid = maxid + i + 2
            theatom = pt.Atom(
                name="CL", charge=0.04, mass=17.0, resname="BOX", type="H", resid=therid
            )
    ...
    def __init__(self, pdbcode, misatodir, **kwarg):
        ...
        if not os.path.exists(self.topfile):
            raise FileNotFoundError(
                f"The topology file of PDB {pdbcode} is not found ({self.topfile})"
            )

        self.trajfile = os.path.join(misatodir, "MD.hdf5")
        if not os.path.exists(self.trajfile):
            raise FileNotFoundError(
                f"The trajectory file is not found ({self.trajfile})"
            )
        ...
        if config.verbose():
            log(
                f"{self.__class__.__name__}: Loading trajectory {pdbcode} with topology {self.topfile}"
            )
        ...
        with h5py.File(self.trajfile, "r") as hdf:
            ...
            if pdbcode.upper() in keys:
                ...
                if config.verbose():
                    log(
                        f"{self.__class__.__name__}: Trajectory loaded with {coord.shape[0]} frames and {coord.shape[1]} atoms"
                    )
                ...
                elif "frame_indices" in kwarg and kwarg["frame_indices"] is not None:
                    slice_frame = np.s_[kwarg["frame_indices"]]
                else:
                    slice_frame = np.s_[:]
                if "mask" in kwarg and kwarg["mask"] is not None:
                ...
                raise ValueError(
                    f"Not found the key for PDB code {pdbcode.upper()} in the HDF5 trajectory file."
                )
        ...
        if kwarg.get("superpose", False):
            if kwarg.get("mask", None) is not None:
                ...
                log(
                    f"{self.__class__.__name__}: Superpose the trajectory with default mask @CA"
                )
                pt.superpose(ret_traj, mask="@CA")

        assert ret_traj.xyz.shape.__len__() == 3, (
            f"Fatal: The shape of the trajectory {self.identity} is {ret_traj.xyz.shape}!!! It should be (n_frames, n_atoms, 3). "
        )
