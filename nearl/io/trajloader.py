...
class TrajectoryLoader:
    ...
    **kwarg : dict
    ...
    def __init__(self, trajs=None, trajtype=None, trajids=None, **kwarg):
        ...
        self.trajs = []
        if isinstance(trajs, (list, tuple)) or hasattr(trajs, "__iter__"):
            for traj in trajs:
        ...
        if trajids is not None:
            if len(trajids) != len(self.trajs):
                raise ValueError(
                    f"The number of trajectory ids {len(trajids)} does not match the number of trajectories {len(self.trajs)}"
                )
        ...
        if kwarg.get("strides", None) is None:
            ...
            if len(kwarg.get("strides")) != len(self.trajs):
                raise ValueError(
                    f"The manually defined number of strides {len(kwarg.get('strides'))} does not match the number of trajectories {len(self.trajs)}"
                )
        ...
        if kwarg.get("masks", None) is None:
            ...
            if len(kwarg.get("masks")) != len(self.trajs):
                raise ValueError(
                    f"The manually defined number of masks {len(kwarg.get('masks'))} does not match the number of trajectories {len(self.trajs)}"
                )
    ...
    def loading_options(self):
        ...
        options = {
            key: value
            for key, value in self.__loading_options.items()
            if value is not None
        }
        if config.verbose or config.debug:
            log(
                f"{self.__class__.__name__}: Loading the trajectory {self.i_} whose identity is {self.trajids[self.i_]}"
            )
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, int):
            if config.verbose or config.debug:
                log(
                    f"{self.__class__.__name__}: Setting the trajectory index to {index}"
                )
            self.i_ = index
    ...
    def append(self, trajs=None, trajtype=None, **kwarg):
        ...
        traj_nr = len(trajs)
        if traj_nr == 0:
            raise ValueError("No trajectory detected in the input")
