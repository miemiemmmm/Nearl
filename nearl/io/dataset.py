...
class Dataset:
    ...
    def __getitem__(self, index):
        ...
        if self.label_key is None:
            ...
            label = torch.tensor(
                [readdata(self.filename(index), self.label_key, self.position(index))]
            )
        return data, label
            ...
            for batch in batches:
                ...
                if self.label_key is None:
                    ...
                    labels = pool.starmap(
                        readdata,
                        [
                            (self.filename(i), self.label_key, self.position(i))
                            for i in batch
                        ],
                    )
                ...
                if augment:
                    data = data_augment(
                        data, trans=augment_translation, add_noise=augment_add_noise
                    )
                yield data, label
