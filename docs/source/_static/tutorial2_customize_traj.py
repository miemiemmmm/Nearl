...
if __name__ == "__main__":
  ...
  """
    ...
    if not os.path.exists(f"{sys.argv[1]}/MD.hdf5"):
        print(
            f"Error: The directory {sys.argv[1]} does not contain the MD.hdf5 file.",
            file=sys.stderr,
        )
        print(f"Usage: {sys.argv[0]} /path/to/misato_dataset", file=sys.stderr)
        sys.exit(1)
    ...
    FEATURIZER_PARMS = {
        ...
        "lengths": 20,
        "time_window": 10,
        "outfile": "/tmp/example.h5",
    }
    loader = nearl.io.TrajectoryLoader(
        trajlist, trajtype=MisatoTraj, superpose=True, trajids=pdbs
    )
    feat = nearl.featurizer.Featurizer(FEATURIZER_PARMS)
    feat.register_trajloader(loader)
    feat.register_focus([":MOL"], "mask")

    feature = nearl.features.Mass(
        selection="!:MOL", outkey="feat_static", cutoff=2.5, sigma=1.0
    )
