...
def test_benchmark_frame_voxelize(benchmark):
    ...
    def run_benchmark():
        ret = commands.frame_voxelize(
            tmp_coords, tmp_weights, dims, spacing, cutoff, sigma
        )
...
def test_benchmark_marching_observer(benchmark):
    ...
    def run_benchmark():
        ret = commands.marching_observer(
            tmp_traj, tmp_weights, dims, spacing, cutoff, 1, 1
        )
        return ret.reshape(dims)
...
def test_benchmark_voxel_dim16(benchmark):
    ...
    tmp_coords = np.random.randint(-cutoff, dim0 + cutoff, size=(atom_nr, 3)).astype(
        np.float32
    )
    tmp_weights = np.full((atom_nr,), 1.5, dtype=np.float32)
    ...
    def run_benchmark():
        ret = commands.frame_voxelize(
            tmp_coords, tmp_weights, dimension, spacing, cutoff, sigma
        )
...
def test_benchmark_mo_dim16(benchmark):
    dim0 = 16
    dimension = np.array([dim0, dim0, dim0], dtype=np.int32)
    tmp_traj = np.random.randint(
        -cutoff, dim0 + cutoff, size=(frame_nr, atom_nr, 3)
    ).astype(np.float32)
    tmp_weights = np.full((frame_nr * atom_nr,), 1.25, dtype=np.float32)

    def run_benchmark():
        ret = commands.marching_observer(
            tmp_traj, tmp_weights, dimension, spacing, cutoff, 1, 1
        )
        return ret.reshape(dimension)
...
def test_benchmark_pdf_dim16(benchmark):
    ...
    def run_benchmark():
        ret = commands.density_flow(
            tmp_coords, tmp_weights, dimension, spacing, cutoff, sigma, 1
        )
        return ret.reshape(dimension)
